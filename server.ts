    ...
    try {
      ...
      if (Array.isArray(pedidos) && Array.isArray(poRest)) {
        ...
        pedidos.forEach(p => {
          ...
          if (poObj) {
            p.createdBy = poObj.createdBy || p.codigoComprador || ""; // Fallback baseline (Comprador que aprovou/emitou)
            const note = poObj.internalNotes || poObj.notes || "";
            ...
            await Promise.all(batch.map(async (reqId) => {
              try {
                ...
                if (res && res.data && res.data.requesterUser) {
                  solicitantesCache[reqId] = res.data.requesterUser;
                }
              } catch(e) {
        ...
        pedidos.forEach(p => {
          if (p.reqIdOrigin && solicitantesCache[p.reqIdOrigin]) {
            ...
            p.solicitante = p.createdBy;
            p.requesterId = p.createdBy;
          }
        });
      }
